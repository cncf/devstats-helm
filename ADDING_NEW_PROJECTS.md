...
- Update (optional) `devstats-helm/values.yaml` cronjob schedules using `devstatscode:splitcrons.sh` script. The default (new) algorithm probes which cronjobs actually exist in the cluster and their DB sizes, then splits the sync/affs schedule spaces weighted by DB size (weight = size^0.5). Defaults need no extra envs: `SPLIT_ALGO=geom` (also: `prop`, `invgeom`, or `WEIGHT_POWER=0.0-4.0`), `SIZES_POD=devstats-postgres-0`, `CTX_TEST=test`/`CTX_PROD=prod` kubectl contexts (set e.g. `CTX_TEST=linode-test` when running outside the cluster, `-` = current context). Projects too big to sync every 6h are excluded from the hourly split and scheduled once per day after the affiliations import instead, with `GHA2DB_RECENT_RANGE`/`GHA2DB_ORPHAN_COMMITS_RANGE` patched to `DAILY_RANGE` (default `26 hours`) and saved as `recentRange`/`orphanCommitsRange` in `values.yaml`: `DAILY_PROJECTS='kubernetes,all,jenkins,opentelemetry,allcdf,istio'` is the default list (use `-` for none). Use `NEVER_PATCH=1` for a dry run (no kubectl patches, still writes the output yaml), `OLD_ALGORITHM=1` for the legacy index-based spread. Typical: `MONTHLY=1 ./splitcrons devstats-helm/values.yaml new-values.yaml`. Note: `new-values.yaml` is marshaled without comments - do not copy it over `values.yaml`; port only the changed `cronTest`/`cronProd`/`affCronTest`/`affCronProd`/`recentRange`/`orphanCommitsRange` lines, then `git add .; git commit -asm "New cron schedules"; git push`.
- Env patching (optional): `[ONLY_ENV=1] [PATCH_ENV='AffSkipTemp,MaxHist,SkipAffsLock,AffsLockDB,NoDurable,DurablePQ,MaxRunDuration,SkipGHAPI,SkipGetRepos,NCPUs,RecentRange,OrphanCommitsRange'] ./splitcrons devstats-helm/values.yaml new-values.yaml`.
