...
- `git pull`.
- Make sure you use test images, not prod, add `affiliationsImage='lukaszgryglicki/devstats-test-rust',provisionImage='lukaszgryglicki/devstats-test-rust',syncImage='lukaszgryglicki/devstats-minimal-test-rust'`.
...
- Update (optional) `devstats-helm/values.yaml` cronjob schedules using `devstatscode:splitcrons.sh` script. The default (new) algorithm probes which cronjobs actually exist in the cluster and their DB sizes, then splits the sync/affs schedule spaces weighted by DB size (weight = size^0.5). Defaults need no extra envs: `SPLIT_ALGO=geom` (also: `prop`, `invgeom`, or `WEIGHT_POWER=0.0-4.0`), `SIZES_POD=devstats-postgres-0`, `CTX_TEST=test`/`CTX_PROD=prod` kubectl contexts (set e.g. `CTX_TEST=linode-test` when running outside the cluster, `-` = current context). Projects too big to sync every 6h are excluded from the hourly split and scheduled once per day after the affiliations import instead, with `GHA2DB_RECENT_RANGE`/`GHA2DB_ORPHAN_COMMITS_RANGE` patched to `DAILY_RANGE` (default `26 hours`) and saved as `recentRange`/`orphanCommitsRange`/`recentReposRange` in `values.yaml`: `DAILY_PROJECTS='kubernetes,all,jenkins,opentelemetry,allcdf,istio'` is the default list (use `-` for none). Each project's affs cron is anchored to its own sync cron so they never start close to each other: regular projects mid-gap between sync slots (+3h for 6-hourly), daily projects sync +`DAILY_AFFS_OFFSET_HOURS` (default 8: the day splits into max 8h for the daily sync + max 16h for the longer affs run); the affs day of month/week still comes from the weighted spread (`NO_AFFS_ANCHOR=1` for the legacy independent placement). Projects with `archived: true` are excluded from all splits (their leftover cronjobs get suspended). Use `NEVER_PATCH=1` for a dry run (no kubectl patches, still writes the output yaml), `OLD_ALGORITHM=1` for the legacy index-based spread. Typical: `MONTHLY=1 ./splitcrons devstats-helm/values.yaml new-values.yaml`. Note: `new-values.yaml` is marshaled without comments - do not copy it over `values.yaml`; port only the changed `cronTest`/`cronProd`/`affCronTest`/`affCronProd`/`recentRange`/`orphanCommitsRange`/`recentReposRange` lines, then `git add .; git commit -asm "New cron schedules"; git push`.
- Env patching (optional): `[ONLY_ENV=1] [PATCH_ENV='AffSkipTemp,MaxHist,SkipAffsLock,AffsLockDB,NoDurable,DurablePQ,MaxRunDuration,SkipGHAPI,SkipGetRepos,NCPUs,RecentRange,OrphanCommitsRange,RecentReposRange'] ./splitcrons devstats-helm/values.yaml new-values.yaml`.
...
- Typical: `` MONTHLY=1 ONLY_PROD=1 ./splitcrons devstats-helm/values.yaml new-values.yaml; vim devstats-helm/values.yaml new-values.yaml; git add .; git commit -asm "New cron schedules"; git push ``. The default (new) algorithm probes alive cronjobs and DB sizes and splits schedules weighted by size - see the test section above for `SPLIT_ALGO`, `DAILY_PROJECTS`/`DAILY_RANGE`, `CTX_PROD`, `NEVER_PATCH` and the warning about not copying `new-values.yaml` wholesale (it drops comments).
...
- If you want to merge multiple new projects (added with `skipAddAll=1`) into `All CNCF` create manual bootstrap debug pod: `helm install devstats-prod-debug ./devstats-helm --set namespace='devstats-prod',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipProvisions=1,skipCrons=1,skipAffiliations=1,skipGrafanas=1,skipServices=1,skipPostgres=1,skipIngress=1,skipStatic=1,skipAPI=1,skipNamespaces=1,bootstrapPodName=debug,bootstrapCommand=sleep,bootstrapCommandArgs={360000s}`. Then shell into it: `../devstats-k8s-lf/util/pod_shell.sh debug`, run: `[ONLY_TABLES/SKIP_TABLES="table1,...,tableN"] GHA2DB_INPUT_DBS="proj1,proj2,..." GHA2DB_OUTPUT_DB="allprj" USE_BATCH=1 BATCH_SIZE=500 PARALLEL=4 merge_dbs && PG_DB="allprj" ./devel/remove_db_dups.sh && exit`. Delete pod: `helm delete devstats-prod-debug`.
- Note: `./devel/remove_db_dups.sh` (via `util_sql/remove_dups.sql`) deduplicates `gha_texts` and `gha_issues_pull_requests` in-place (a `DELETE` keeping one row per group of fully identical rows) - it never drops/recreates the tables, takes no exclusive lock (tables stay fully readable/writable during dedup) and therefore cannot lose indexes, grants or statistics. Its final step runs `util_sql/ensure_texts_iprs_indexes.sql` (`create index if not exists` for the canonical index set from `structure.go`), which also heals databases deduplicated by older script versions - those used bare `CREATE TABLE (LIKE ...)` + swap and silently dropped all indexes and grants on every run (this is exactly how `allprj` ended up with unindexed `gha_texts`/`gha_issues_pull_requests`).
- If redoing this because All CNCF lacks something you can safely skip those tables: `` SKIP_TABLES='gha_actors,gha_texts' ... ```. Or specify syncing commits, issues, PRs: `` ONLY_TABLES='gha_reviews,gha_commits,gha_commits_files,gha_commits_roles,gha_events,gha_issues,gha_issues_assignees,gha_issues_events_labels,gha_issues_labels,gha_issues_pull_requests,gha_payloads,gha_pull_requests,gha_pull_requests_assignees,gha_pull_requests_requested_reviewers' ... ``. To merge all DBs use: `` GHA2DB_INPUT_DBS="-all-" GHA2DB_OUTPUT_DB="allprj" SKIP_TABLES="gha_actors,gha_texts" merge_dbs ``. Another example: `` GHA2DB_INPUT_DBS="-all-" GHA2DB_OUTPUT_DB="allprj" ONLY_TABLES="gha_teams,gha_orgs" GHA2DB_LOCAL=1 SKIP_DBS="smi,keptn" IGNORE_NO_DB=1 merge_dbs ``.
- After merging update repo groups definition via: `` k exec -in devstats-prod devstats-postgres-0 -c devstats-postgres -- psql allprj < ~/cncf/devstats/scripts/all/repo_groups.sql ``.
...
- Recreate ingress with a new hostname (verified 2026-08 during SDC add): do NOT `helm delete` the release (takes ALL hosts down, apex included). Instead pre-delete only the project shards (keep `devstats-ingress-1` apex up): `for i in 2 3 4 5 6 7 8; do kubectl delete ingress -n devstats-prod devstats-ingress-$i; done`, then `helm upgrade devstats-prod-ingress ./devstats-helm --force-conflicts --set namespace='devstats-prod',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipBootstrap=1,skipProvisions=1,skipCrons=1,skipAffiliations=1,skipGrafanas=1,skipServices=1,skipPostgres=1,skipStatic=1,skipAPI=1,skipNamespaces=1,indexDomainsFrom=1,skipAliases=1,ingressClass=nginx-prod,sslEnv=prod,backupsPageIngress=1`. Notes: `backupsPageIngress=1` is REQUIRED on prod (live apex shard has `/backups` paths; omitting it silently strips them). `--force-conflicts` is required with helm v4 SSA when any ingress was ever touched by manual `kubectl patch` (field-manager conflict fails the upgrade otherwise). The nginx admission webhook denies any apply that adds a host already present in ANOTHER ingress, so in-place re-sharding always fails without the pre-delete; worse, a failed upgrade can PARTIALLY apply (pure host-removal patches pass the webhook and land even though the release shows FAILED) leaving hosts silently serving the default backend - after ANY failed ingress upgrade, diff live objects against `helm get manifest`. Downtime is limited to project hosts for ~2 min (Let's Encrypt re-issues all certs in ~100 s thanks to <30-day authorization reuse). Verify: compare live ingresses vs `helm template` render (hosts+tls per shard) and SNI-probe every host: `openssl s_client -servername $h -connect <LB-IP>:443 </dev/null 2>/dev/null | openssl x509 -noout -ext subjectAltName | grep DNS:$h`.
- Redeploy All CNCF Grafana (new project in health dashboards): `kubectl edit deployment devstats-grafana-all` - change `image:` add or remove `:latest` which will force rolling update without downtime.
- Run vars regenerate on all projects (excluding new one, this is needed to update home dahsboard's list of all projects): `helm install --generate-name ./devstats-helm --set namespace='devstats-prod',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipBootstrap=1,skipCrons=1,skipAffiliations=1,skipGrafanas=1,skipServices=1,skipPostgres=1,skipIngress=1,skipStatic=1,skipAPI=1,skipNamespaces=1,testServer='',prodServer='1',provisionImage='lukaszgryglicki/devstats-prod-rust',provisionCommand='devstats-helm/vars.sh',useFlagsProv='',provisionPodName=devstats-provision-vars`.
- If you used a DB backup from a `test` you also need to rerun vars on restored DB (it has test-specific varaibles set not the prod ones): `helm install --generate-name ./devstats-helm --set namespace='devstats-prod',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipBootstrap=1,skipCrons=1,skipAffiliations=1,skipGrafanas=1,skipServices=1,skipPostgres=1,skipIngress=1,skipStatic=1,skipAPI=1,skipNamespaces=1,testServer='',prodServer='1',provisionImage='lukaszgryglicki/devstats-prod-rust',provisionCommand='devstats-helm/vars.sh',useFlagsProv='',indexProvisionsFrom=96,provisionPodName=xyz`.
- Update All CNCF repo groups definitions (not needed if planning to run reinit for All CNCF - which is typical): `helm install --generate-name ./devstats-helm --set namespace='devstats-prod',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipBootstrap=1,skipCrons=1,skipAffiliations=1,skipGrafanas=1,skipServices=1,skipPostgres=1,skipIngress=1,skipStatic=1,skipAPI=1,skipNamespaces=1,provisionImage='lukaszgryglicki/devstats-prod-rust',indexProvisionsFrom=38,indexProvisionsTo=39,provisionCommand='./devstats-helm/repo_groups.sh',useRepos=1,skipECFRGReset=1`.
- Eventually update All CNCF tags definitions (will break dashboards for a while, not needed if running reinit): `helm install --generate-name ./devstats-helm --set namespace='devstats-prod',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipBootstrap=1,skipCrons=1,skipAffiliations=1,skipGrafanas=1,skipServices=1,skipPostgres=1,skipIngress=1,skipStatic=1,skipAPI=1,skipNamespaces=1,provisionImage='lukaszgryglicki/devstats-prod-rust',indexProvisionsFrom=38,indexProvisionsTo=39,provisionCommand='./devstats-helm/tags.sh'`.
- Eventually reinit All CNCF (recommended): `helm install --generate-name ./devstats-helm --set namespace='devstats-prod',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipBootstrap=1,skipCrons=1,skipAffiliations=1,skipGrafanas=1,skipServices=1,skipPostgres=1,skipIngress=1,skipStatic=1,skipAPI=1,skipNamespaces=1,provisionImage='lukaszgryglicki/devstats-prod-rust',indexProvisionsFrom=38,indexProvisionsTo=39,provisionCommand='./devstats-helm/reinit.sh',ghaAPISkip=1,giantProv='',skipRand=1,nCPUs=6,allowMetricFail=1`.
- In case of provisioning failure you can recreate failed provisioning pod via: `helm install --generate-name ./devstats-helm --set namespace='devstats-prod',provisionImage='lukaszgryglicki/devstats-prod-rust',testServer='',prodServer='1',skipSecrets=1,skipPVs=1,skipBackupsPV=1,skipVacuum=1,skipBackups=1,skipBootstrap=1,skipCrons=1,skipGrafanas=1,skipServices=1,skipAffiliations=1,skipPostgres=1,skipIngress=1,skipStatic=1,skipAPI=1,skipNamespaces=1,projectsOverride='+cncf\,+opencontainers\,+zephyr\,+linux\,+rkt\,+sam\,+azf\,+riff\,+fn\,+openwhisk\,+openfaas\,+cii\,+prestodb\,+godotengine\,+opentracing',skipECFRGReset=1,skipAddAll=1,provisionCommand=sleep,provisionCommandArgs={360000s},provisionPodName=fix,indexProvisionsFrom=P,indexProvisionsTo=P+1,nCPUs=8` and then inside the `k exec -it fix-projname -- bash`, `vi proj-name/psql.sh`, finally run `WAITBOOT=1 ORGNAME="-" PORT="-" ICON="-" GRAFSUFF="-" GA="-" SKIPGRAFANA=1 PDB=1 TSDB=1 GHA2DB_MGETC=y ./proj-name/psql.sh`.
- Then: `WAITBOOT=1 ORGNAME="-" PORT="-" ICON="-" GRAFSUFF="-" GA="-" SKIPGRAFANA=1 PDB=1 TSDB=1 GHA2DB_MGETC=y ./devel/ro_user_grants.sh "metallb"`, `WAITBOOT=1 ORGNAME="-" PORT="-" ICON="-" GRAFSUFF="-" GA="-" SKIPGRAFANA=1 PDB=1 TSDB=1 GHA2DB_MGETC=y ./devel/psql_user_grants.sh "devstats_team" "metallb"`, finally: `WAITBOOT=1 ./devstats-helm/deploy_all.sh`.
...
- Update `BigQuery/velocity_lf.sql BigQuery/velocity_cncf.sql BigQuery/velocity_condition_lf.sql BigQuery/velocity_condition_cncf.sql`.
- Update `map/hints.csv map/defmaps.csv map/urls.csv map/ranges_sane.csv`.
- All at once: `vim reports/cncf_projects_config.csv BigQuery/velocity_lf.sql BigQuery/velocity_cncf.sql BigQuery/velocity_condition_lf.sql BigQuery/velocity_condition_cncf.sql map/hints.csv map/defmaps.csv map/urls.csv map/ranges_sane.csv`.
