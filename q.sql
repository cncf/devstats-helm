...
where
   (c.dup_created_at >= now() - '1 day'::interval) 
...
where
  ...
  and  (cr.dup_created_at >= now() - '1 day'::interval) 
...
where
   (e.created_at >= now() - '1 day'::interval) 
;
...
where
   (c.created_at >= now() - '1 day'::interval) 
;
...
where
   (i.created_at >= now() - '1 day'::interval) 
;
...
where
  ...
  and  (pr.merged_at >= now() - '1 day'::interval) 
;
